...
public class StandardSecurityConfig {
    ...
    public SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.csrf(csrf -> csrf.ignoringRequestMatchers("/**"))
            .authorizeHttpRequests(authorize -> authorize
                .requestMatchers("/oauth2/**", "/login", "/webjars/**")
                .permitAll()
                .anyRequest()
                .authenticated())
    ...
    private void setResponseMetadata(HttpServletResponse response, HttpStatus httpStatus, String message)
        throws IOException {
        ...
        response.getWriter()
            .write(objectMapper.writeValueAsString(new ExceptionResponseBody(httpStatus.value(), message)));
    }

    private AccessDeniedHandler accessDeniedHandler() {
        return (request, response, accessDeniedException) -> setResponseMetadata(response, HttpStatus.FORBIDDEN, "Access is denied, you do not have permission to access this resource");
    }
